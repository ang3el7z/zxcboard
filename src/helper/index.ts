...
import { hiddenGroupMap, proxyGroupList, proxyMap } from '@/store/proxies'
import {
  customThemes,
  displayAllFeatures,
  lowLatency,
...
export const renderRoutes = computed(() => {
  ...
  const routes = displayAllFeatures.value
  ...
  return routes.filter((r) => {
    ...
    if (r === ROUTE_NAME.proxies && !isProxiesRouteVisible()) return false
    if (r in routeCapable && routeCapable[r] === false) return false
    return true
  })
})

const isProxiesRouteVisible = () => {
  if (manageHiddenGroup.value) {
    return true
  }

  if (Object.keys(proxyMap.value).length === 0) {
    return true
  }

  return proxyGroupList.value.some((group) => !isHiddenGroup(group))
}
