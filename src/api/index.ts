...
const ignoreNotificationUrls = ['/delay', '/weights', '/storage/zxcboard']
...
export const zxcboardVersion = ref(__APP_VERSION__)
...
export const getStorageAPI = () => {
  return axios.get<Record<string, unknown>>(`/storage/zxcboard`)
}

export const setStorageAPI = (value: Record<string, string>) => {
  return axios.put(`/storage/zxcboard`, value)
}

export const deleteStorageAPI = () => {
  return axios.delete(`/storage/zxcboard`)
}
...
export const fetchIsUIUpdateAvailable = async () => {
  const { tag_name } = await fetchWithLocalCache<{ tag_name: string }>(
    'https://api.github.com/repos/ang3el7z/zxcboard/releases/latest',
    zxcboardVersion.value,
  )

  return Boolean(tag_name && tag_name !== `v${zxcboardVersion.value}`)
}
