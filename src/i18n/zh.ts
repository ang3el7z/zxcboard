...
const zh: LANG_MESSAGE = {
  ...
  unavailable: '不可用',
  protocolTips:
    '您正在尝试连接一个 HTTP 后端，但 zxcboard 是通过 HTTPS 提供的，这可能会导致连接错误。请在浏览器设置中允许不安全内容，或者通过 HTTP 打开 zxcboard。',
  global: '全局',
  direct: '直连',
  ...
  autoDisconnectIdleUDPTip:
    '启用后，打开 zxcboard 时将关闭持续时间超过配置分钟数的 UDP 连接，这可能对某些无法正常自动断开的 UDP 连接带来帮助。(这是作者自己的私货，你大概率不需要打开它)',
  ...
  importFromBackendTip:
    '默认的 ./zxcboard-settings.json 位于 ui 文件夹下，导入前请确保 ui 文件夹下存在配置文件。',
  importFailed: '导入失败，请检查url {url}',
  autoImportFromUrl: '自动导入',
  autoImportFromUrlTip:
    '启用后，每次打开 zxcboard 时会自动从 URL 设置导入并计算 hash，如果 hash 与上次导入的 hash 不同，则重新导入并刷新页面。',
  getting: '获取中...',
