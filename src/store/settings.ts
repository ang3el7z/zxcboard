...
import type { LatencyServiceKey } from '@/api/latency'
import { LATENCY_SERVICE_CONFIG } from '@/api/latency'
import {
  ALL_THEME,
...
// overview
const migrateSplitOverviewDefault = () => {
  if (typeof window === 'undefined') {
    return
  }

  const migrationKey = 'config/split-overview-page-default-enabled'
...
export const defaultLatencyServices: LatencyServiceKey[] = [
  'google',
  'cloudflare',
  'youtube',
  'github',
...
export const enabledLatencyServices = useStorage<LatencyServiceKey[]>(
  'config/enabled-latency-services',
  defaultLatencyServices,
)
enabledLatencyServices.value = enabledLatencyServices.value.filter((service) => {
  return service in LATENCY_SERVICE_CONFIG
})
