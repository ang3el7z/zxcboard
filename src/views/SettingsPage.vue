...
import ZxcboardSettings from '@/components/settings/general/ZxcboardSettings.vue'
...
import { displayAllFeatures, settingsMenuOrder, settingsPageTwoColumns } from '@/store/settings'
import {
  ArrowPathIcon,
  ArrowsRightLeftIcon,
  CubeTransparentIcon,
...
const menuItems = computed<MenuItem[]>(() => {
  ...
  return settingsMenuOrder.value
    ...
    .filter((item): item is MenuItem => {
      if (!item || !isSettingVisible(item.key)) {
        return false
      }

      if (!displayAllFeatures.value && item.key === SETTINGS_MENU_KEY.backend) {
        return false
      }
