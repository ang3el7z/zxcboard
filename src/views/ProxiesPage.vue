      ...
      >
        <template v-if="showEmptyState">
          <div class="flex h-full items-center justify-center p-4">
            <div class="bg-base-200/40 flex max-w-md flex-col items-center gap-3 rounded-2xl px-6 py-8 text-center">
...
const showProviderTab = computed(() => {
  return displayAllFeatures.value && proxyProviederList.value.length > 0 && hasVisibleProxyGroups.value
})

watch(
  showProviderTab,
  (value) => {
    if (!value && proxiesTabShow.value === PROXY_TAB_TYPE.PROVIDER) {
...
const showEmptyState = computed(() => {
  return proxiesTabShow.value === PROXY_TAB_TYPE.PROXIES && !hasVisibleProxyGroups.value
})
